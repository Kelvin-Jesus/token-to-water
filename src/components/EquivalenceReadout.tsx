...
export function EquivalenceReadout({ liters, tokens, mlPerToken, position, equivalence, className }: EquivalenceReadoutProps) {
  ...
  return (
    // @container: the headline number is sized from the card width, so long values ("3.75 million km³")
    // shrink instead of overflowing, and it never wraps — a still layout while the slider is dragged.
    <section aria-labelledby="readout-heading" className={cn('@container flex flex-col gap-4', className)}>
      <div className="flex flex-col gap-1">
        <h2 id="readout-heading" className="text-sm font-medium text-muted-foreground">
          {t('readout.heading')}
        </h2>
        <p
          className="flex items-baseline gap-[0.15em] text-[min(3.75rem,11cqi)] leading-none font-semibold tracking-tight whitespace-nowrap tabular-nums"
          data-testid="volume"
        >
          <span>{volume.value}</span>
          <span className="text-[0.5em] text-muted-foreground">{volume.unit}</span>
        </p>
        {/* Two lines reserved: sentences alternate between one and two lines while the slider moves. */}
        <p className="mt-1 min-h-[2lh] text-lg leading-snug font-medium text-balance sm:text-xl" data-testid="equivalence">
          {sentence}
        </p>
      </div>
      ...
      <div className="flex items-center gap-3 rounded-xl bg-muted/70 px-3 py-2.5">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1.5">
          {/* One line either way: a second label beside this one wrapped on phones and resized the card. */}
          <p className={cn('truncate text-[0.8125rem]', beyondEarth ? 'font-medium text-primary' : 'text-muted-foreground')}>
            {beyondEarth ? t('readout.beyond') : t('readout.current')}
          </p>
      ...
      <p className="text-xs text-muted-foreground tabular-nums max-sm:min-h-[2lh]">
